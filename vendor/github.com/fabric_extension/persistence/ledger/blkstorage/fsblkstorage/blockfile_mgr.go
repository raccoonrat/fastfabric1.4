...
import (
	"fmt"
	"github.com/fabric_extension/persistence/ledger/blkstorage"
	"github.com/fabric_extension/persistence/ledger/util/leveldbhelper"
	"math"
	"sync"
	"sync/atomic"
...
func newBlockfileMgr(id string, conf *Conf, indexConfig *blkstorage.IndexConfig, indexStore *leveldbhelper.DBHandle) *blockfileMgr {
	...
	rootDir := "/var/hyperledger/" + conf.getLedgerBlockDir(id)
	_, err := util.CreateDirIfMissing(rootDir)
	fmt.Println("newBlockfileMgr:", rootDir)
	if err != nil {
