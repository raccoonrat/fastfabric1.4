...
func (cache blockCache) Put(block *common.Block) error {
	if block == nil || block.Data == nil || block.Data.Data == nil{
		return errors.New("The block argument for a cache put must not be nil.")
	}
...
func (cache blockCache) Get(blockNo uint64) (*UnmarshaledBlock, error) {
	...
	if block.Raw.Header.Number != blockNo {
		return nil, errors.New(fmt.Sprintln("Found block", block.Raw.Header.Number, "instead"))
	}
...
func newBlock(rawBlock *common.Block) *UnmarshaledBlock {
	b := &UnmarshaledBlock{Raw: rawBlock, Txs: make([]*UnmarshaledTx, len(rawBlock.Data.Data))}
	for idx, data := range b.Raw.Data.Data {
...
func (b *UnmarshaledBlock) GetMetadata() (*common.Metadata, error) {
	if b.meta == nil && b.metaErr == nil {
		b.meta, b.metaErr = utils.GetMetadataFromBlock(b.Raw, common.BlockMetadataIndex_SIGNATURES)
	}
...
func (act *UnmarshalledAction) GetDeploymentSpec() (*peer.ChaincodeDeploymentSpec, error) {
	if act.depSpec == nil && act.depErr == nil {
		...
		if inv, act.depErr = act.GetInvokeSpec(); act.depErr == nil {
			if inv != nil {
				act.depSpec, act.depErr = utils.GetChaincodeDeploymentSpec(inv.ChaincodeSpec.Input.Args[2],platforms.NewRegistry(&golang.Platform{}))
			}
		}
	}
...
func (b *UnmarshaledBlock) GetChannelId() (string, error) {
	if len(b.Txs) == 0{
		return "", fmt.Errorf("Block [%d] has no transactions", b.Raw.Header.Number)
	}

	if chdr, err :=  b.Txs[0].GetChannelHeader(); err != nil{
		return "", err
	}else{
...
func (b *UnmarshaledBlock) VerifyHash() error  {
	if b.datahash == nil {
		b.datahash = b.Raw.Data.Hash()
	}

	if !bytes.Equal(b.datahash, b.Raw.Header.DataHash ) {
		chId, _ := b.GetChannelId()
		return fmt.Errorf("Header.DataHash is different from Hash(block.Data) for block with id [%d] on channel [%s]", b.Raw.Header.Number, chId)
	}
	return nil
}